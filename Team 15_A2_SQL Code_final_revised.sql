...
-- Calculating current assets
	...
	SELECT
		   SUM(IFNULL(jeli.credit,0))-SUM(IFNULL(jeli.debit,0))  INTO varCurrent_Assets
	FROM `account` as a
...
-- Calculating Fixed Assets
	...
	SELECT
        ...
        IFNULL(((SUM(IFNULL(jeli.credit,0)))-SUM(IFNULL(jeli.debit,0))),0) INTO varFixed_Assets
	FROM `account` as a
	LEFT JOIN statement_section as ss
	ON a.balance_sheet_section_id = ss.statement_section_id
	INNER JOIN journal_entry_line_item AS jeli
...
-- Calculating LY current assets
	...
	SELECT
		   (SUM(IFNULL(jeli.credit,0))-SUM(IFNULL(jeli.debit,0)))  INTO varCurrent_AssetsLY
	FROM `account` as a
	LEFT JOIN statement_section as ss
	ON a.balance_sheet_section_id = ss.statement_section_id
	INNER JOIN journal_entry_line_item AS jeli
...
-- Calculating LY Fixed Assets
	...
	SELECT
		IFNULL((SUM(IFNULL(jeli.credit,0))-SUM(IFNULL(jeli.debit,0))),0) INTO varFixed_AssetsLY
	FROM `account` as a
	LEFT JOIN statement_section as ss
	ON a.balance_sheet_section_id = ss.statement_section_id
	INNER JOIN journal_entry_line_item AS jeli
